...
- Température de retour : `min_return_temp = 40 °C`, un consommateur ne pourra pas soutirer de chaleur au réseau si la température est inférieure ou égale à cette valeur.
- Température initiale du réseau : lors du démarrage du réseau, à $t=0$, la température de toutes les cellules du réseau est initialisée à la température minimale de retour, l'agent doit donc apprendre à "démarrer" le réseau et à gérer le régime transitoire.
...
Le cœur du code se trouve dans `district_heating_model.py`. Une approche unidimensionnelle basée sur la méthode des volumes finis est utilisée.
...
**Justification de l'existence de $T_{\text{min return}}$ :** physiquement, le transfert thermique s'arrête quand la température du circuit primaire (réseau de chaleur) atteint celle du circuit secondaire (immeuble).
...
- Pas de contrôle : `dt = 10 s` (l'agent prend une décision toutes les 10s.).
...
- Adapter $T_{in}$ et $\dot m$ pour anticiper les variations de charge.
...
3. **Splits** : répartition des débits dans les branchements.

**Ramping / contraintes dynamiques:**

Pour garantir le réalisme physique et protéger le matériel simulé, des contraintes de variation sont imposées. Elles sont basées sur les limites techniques des chaudières industrielles :

- **Montée en température** : limitée à environ **+0,5 à +1,5 °C par 10 secondes**.
  - Les chaudières eau chaude tolèrent des gradients de 3 à 8 K/min (biomasse) voire 10-15 K/min (gaz rapide), soit environ +0,5 à +2,5 °C/10s [^10].
- **Descente en température** : limitée à environ **-0,2 à -0,8 °C par 10 secondes**.
  - Le refroidissement est plus lent (inertie thermique, pas de refroidissement actif par le brûleur). Une chute brutale (choc thermique à l'eau froide) est dangereuse pour le corps de chauffe [^11].

Ces bornes (`max_temp_rise_per_dt`, `max_temp_drop_per_dt`) empêchent l'agent d'adopter des stratégies trop brutales qui endommageraient une vraie chaufferie.
...
- $P_{\text{ref}} = 2000$ kW  (demande de puissance moyenne)
- $P_{\text{nom}} = 15$ kW

-> ajouter source puissance nominale pompe
...
Les graphes suivants ont été tracés avec une température d'entrée constante `inlet_temp = 70 °C` et un débit massique constant `inlet_mass_flow = 12 kg/s`.

Les fractions massiques au branchement n'ont pas été spécifiées, les débits ont donc été également répartis dans les noeuds concernés (50/50).

Dans cette configuration, les valeurs fixées en entrée sont trop faibles pour combler la demande en puissance. Cela est visible sur les courbes : 
...
Parmi touts les modèles entrainés et toutes les configurations testées, les courbes ci-dessous ont été réalisées avec le plus "performant".

Sur les 4 graphes ci dessous, on remarques 4 choses intéressantes :
- La puissance fournie est deux fois inférieure à la demande, **le modèle est donc exécrable** (même si c'est le meilleur que j'ai réussi à obtenir). Le coefficient de la fonction reward concernant le critère de confort est donc trop faible ? Après quelques essais je ne suis pas convaincu que ce soit uniquement lié à cela. 
- La température initiale du réseau valant `min_return_temp = 40 °C`, on constate que **l'agent a appris à gérer le régime transitoire** (de $t=0$ à $t=3h$).
- La valeur du débit massique autour de laquelle le modèle se stabilise n'est pas anodine : il s'agit de la valeur nominale de la pompe. La fonction reward pénalisant l'utilisation de la pompe à d'autres endroits que son point nominal, le modèle a donc appris à **se stabiliser à ce point de fonctionnement nominal**. A noter que le modèle **sort de cette règle pour la phase transitoire initiale**, où l'utilisation de la pompe à un régime plus élevé est moins pénalisée que la différence de puissance $P_\text{demand}-P_\text{supplied}$.
- La fonction reward n'atteint jamais zéro (la fonction reward a été concue pour que ce soit la valeur maximale théorique). Le modèle se satisfait donc d'un **sous-optimum**.

![](plots/eval_PPO_normalisé1_168.svg)

On peut observer sur le graphe suivant, en fonction du nombre de mises à jour de la politique interne de l'agent, les effets de l'entraînement sur :
